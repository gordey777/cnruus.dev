  ...
  function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if ( $use_desc_for_title == 0 || empty($category->description) )
      ...
      if ( !empty($feed_image) || !empty($feed) ) {
        $link .= ' ';
        if ( empty($feed_image) ) $link .= '('; $link .= '<a href="' . esc_url( get_term_feed_link( $category->term_id, $category->taxonomy, $feed_type ) ) . '"';
        if ( empty($feed) ) {
      ...
      if ( 'list' == $args['style'] ) {
        $output .= "\t<li";
        $class = 'cat-item'; // Your custom class
        if ($depth) $class .= '';
        if ( !empty($current_category) ) {
          $_current_category = get_term( $current_category, $category->taxonomy );
          if ( $category->term_id == $current_category ) $class .= ' current-cat';
          elseif ( $category->term_id == $_current_category->parent ) $class .= ' current-cat-parent';
        }
        $output .= ' class="' . $class . '"';
        $output .= ">$link\n";
      }
